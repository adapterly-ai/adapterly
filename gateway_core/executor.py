...
async def execute_system_tool(
    ...
    try:
        ...
        # Credential resolution — always workspace (project) level, fallback to account level
        account_system_stmt = (
            ...
            .where(AccountSystem.project_id == project_id)
        )
        result = await db.execute(account_system_stmt)
        account_system = result.scalars().first()

        # Fallback: try account-level credentials (transition period)
        if not account_system:
            fallback_stmt = (
                ...
                .where(AccountSystem.system_id == system.id)
                .where(AccountSystem.is_enabled == True)  # noqa: E712
                .where(AccountSystem.project_id == None)  # noqa: E711
            )
            result = await db.execute(fallback_stmt)
            account_system = result.scalars().first()

        if not account_system:
            return {"error": f"No credentials found for {system.alias}"}
